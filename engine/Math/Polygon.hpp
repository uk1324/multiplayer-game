...
auto simplePolygonSignedArea(std::span<const Vec2> verts) -> float;
auto simplePolygonArea(std::span<const Vec2> verts) -> float;
auto simplePolygonIsClockwise(std::span<const Vec2> verts) -> bool;
...
auto lineSegmentIntersectsSimplePolygon(Vec2 start, Vec2 end, std::span<const Vec2> verts) -> bool;

// Can return zero area shapes.
// Can create self intersections.
auto polygonDouglassPeckerSimplify(std::span<const Vec2> verts, float maxDistanceFromLine) -> std::vector<Vec2>;
...
auto isPointInPolygon(std::span<const Vec2> verts, Vec2 p) -> bool;
auto simplePolygonContainsSimplePolygon(std::span<const Vec2> enclosed, std::span<const Vec2> container) -> bool;
...
auto complexPolygonOutline(std::span<const Vec2> vertices, std::vector<Vec2>& result = complexPolygonOtlineResult) -> const std::vector<Vec2>&;