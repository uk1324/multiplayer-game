...
LineTriangulator::Result LineTriangulator::operator()(std::span<const Vec2> line) {
	...
	auto roundJoin = [&](Vec2 start, Vec2 end, Vec2 around, int count, bool aroundIsDown, float xPos) {
		if (!a) return;
		float aroundYPos;
		float arcYPos;
		if (aroundIsDown) {
		...
		for (int i = 0; i < count; i++) {
			...
			vertices.push_back({ around, Vec2(xPos, aroundYPos) });
	...
	for (i32 i = 0; i < static_cast<i32>(line.size()) - 1; i++) {
		...
		if (intersectionUp.has_value()) {
			...
			roundJoin(downVertex.pos, previousDown.pos, *intersectionUp, 5, false, textureX);
			...
			roundJoin(upVertex.pos, previousUp.pos, *intersectionDown, 5, true, textureX);
			distanceAlongLine = textureX;
		} else {
			continue;
