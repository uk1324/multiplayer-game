...
struct ImGuiViewportP : public ImGuiViewport
{
    ...
    // Calculate work rect position/size given a set of offset (we have 1 pair of offset for rect locked from last frame data, and 1 pair for currently building rect)
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    float                   TextWrapPos;            // Current text wrap position.
    ImVector<float>         ItemWidthStack;         // Store item widths to restore (attention: .back() is not == ItemWidth)
    ImVector<float>         TextWrapPosStack;       // Store text wrap position to restore (attention: .back() is not == TextWrapPos)
};
